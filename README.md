...
## 模板职责边界（Scope，基线 v0.1.0 冻结）

本仓库是**工程底座（模板）**，职责到此固定。它**提供**：

- 工作区六边形分层 + 复杂度分档（Cargo features：`minimal`→`full`）
- 配置分层 / 可观测性、统一错误（RFC 7807）、强类型 `Id<T>` / 分页 / 请求上下文
- PostgreSQL 接入 + 多租户 RLS、迁移分档
- 身份联合与会话（密码 + 平台 token 交换 + JIT 映射）、RBAC
- 审计日志 + 事务性发件箱、行级历史
- `Project` 参考切片（业务功能的「复制范本」）、多环境打包（Docker/compose/Helm）、CI、测试骨架

它**不包含**任何具体业务（小分子 / 数据集 / 自定义实体 / 字段级权限的落地等）。这些是**基于本模板构建的下游产品功能**——按 [docs/architecture.md](docs/architecture.md) 复制 `Project` 切片、归入合适的 feature 档实现；相关调研结论见 [docs/notes/](docs/notes/)。

> 基线版本 **v0.1.0**。后续业务在其之上迭代，不再回头改动模板边界（除非确有通用能力需要沉淀回模板）。
