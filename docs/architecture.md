...
| `dms-api` | HTTP 路由、中间件、错误映射、OpenAPI | 依赖 application，不依赖 infrastructure |
...
4. `migrations`：新增表 + RLS 策略 + 审计触发器 + history 表。
5. `api`：挂 `/v1/<feature>` 路由 + utoipa 注解。
6. `bin/server`：在组合根注入新服务。
7. 测试：领域单元 / 服务(mock) / 集成(sqlx::test) / API e2e。
...
- **多环境**：figment 分层 + Cargo features（cloud/onprem）+ 同镜像配置驱动。
